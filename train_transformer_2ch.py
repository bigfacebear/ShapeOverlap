...
train_dir = os.path.join(FLAGS.train_dir, 'transformer_2ch')

def inference(locks, keys, eval=False):
    batch_size, height, width, _ = locks.get_shape().as_list()
    theta = models.two_channel_inference(locks, keys, eval, output_size=3)
    keys = models.rotate_and_translate_transformer(keys, theta, (height, width))
